...
app.controller('BrowseController', function($scope, $routeParams, toaster, Task, Auth, Comment, Offer, $firebaseObject) {
  ...
  if($routeParams.taskId) {
    ...
    // [AngularFire 1.1.2]
    var task = $firebaseObject(Task.getTask($routeParams.taskId));
  ...
  $scope.cancelTask = function(taskId) {
    Task.cancelTask(taskId, function(error) {
      if (error) {
  ...
  $scope.completeTask = function(taskId) {
    Task.completeTask(taskId, function(error) {
      if (error) {
        toaster.pop('error', "Oops, something went wrong.");
      } else {
  ...
  $scope.cancelOffer = function(offerId) {
    Offer.cancelOffer($scope.selectedTask.$id, offerId, function(error) {
      if (error) {
        toaster.pop('error', "Oops, something went wrong.");
      } else {
  ...
  $scope.acceptOffer = function(offerId, runnerId) {
    Offer.acceptOffer($scope.selectedTask.$id, offerId, runnerId, function(error) {
      if (error) {
        toaster.pop('error', "Oops, something went wrong.");
      } else {
